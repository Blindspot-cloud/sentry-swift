...
public struct Envelope {
    ...
    public func dump(encoder: JSONEncoder) throws -> Data {
        ...
        guard returnData.count <= Sentry.maxEnvelopeCompressedSize else {
...
public struct EnvelopeHeader: Encodable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if let eventId = eventId {
            try container.encode(eventId.hexadecimalEncoded, forKey: CodingKeys.eventId)
        }
        if let dsn = dsn {
...
public struct EnvelopeItemHeader: Codable {
    ...
    public init(type: EnvelopItemTypes, length: UInt64 = 0, filename: String?, contentType: String?) {
...
public struct EnvelopeItem {
    ...
    fileprivate let data: Encodable
    public init(header: EnvelopeItemHeader, data: Encodable) {
        self.header = header
        self.data = data
    }
    ...
    public func dump(encoder: JSONEncoder) throws -> Data {
        ...
        let header = EnvelopeItemHeader(type: header.type, length: UInt64(data.count), filename: header.filename, contentType: header.contentType)
