...
public struct Dsn {
    internal static let localhost: Dsn = Dsn(scheme: "http", host: "localhost", port: 3000, projectId: 1, path: "", publicKey: "key", secretKey: nil)
    ...
    public init(fromString value: String) throws {
        ...
        guard let scheme = parsedDsn.scheme, !scheme.isEmpty else {
            throw SentryError.InvalidArgumentException("The \"\(value)\" DSN must contain a scheme, a host, a user and a path component.")
        }
        ...
        if let pass = parsedDsn.password {
            guard !pass.isEmpty else {
                throw SentryError.InvalidArgumentException("The \"\(value)\" DSN must contain a valid secret key.")
            }
        }

        guard scheme == "http" || scheme == "https" else {
            throw SentryError.InvalidArgumentException("The scheme of the \"\(value)\" DSN must be either \"http\" or \"https\".")
        }
        ...
        guard let projectId = Int(segmentPaths.removeLast()), projectId >= 0 else {
            throw SentryError.InvalidArgumentException("\"\(value)\" DSN must contain a valid project ID.")
        }
